...
- warehouse layout optimized for dashboard access by partitioning on `measurement_date` and clustering on `city` and `pollutant`
...
- the project uses GCP as the cloud platform rather than a local-only setup
- Terraform is the infrastructure-as-code layer for the storage and warehouse foundation
- the BigQuery table design explicitly supports the dashboard query pattern
